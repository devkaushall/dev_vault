...
Version 2.7.0 · Built by DEVil · https://github.com/devkaushall
...
are fifteen screens. Every one of them has a **"Need help?"** button in the
...
**What it is for:** the changes your team saw on the ground that the website has
wrong, waiting for you to say yes or no.

A property belongs to whoever added it, and nobody else can edit it. That is the
right rule, but it used to mean that an agent standing in a flat where the owner
had just changed the price could only phone somebody later. Now they fill in one
short form on the property itself — price, rent, whether it is still available,
the description, and a line about how they know. Those requests land here.

**Every request shows two things side by side:** what the website says today and
what is being proposed. A number on its own cannot be judged; a change from
"₹8.5 Cr" to "₹8.2 Cr" can be, in a second.

Then there are two buttons, deliberately far apart:

- **Yes, change it** — the property is updated and the website changes. It goes
  through the same save path as the editor, so the search, the activity history
  and "undo last edit" all behave as if you had typed it yourself.
- **Not now** — nothing changes. You can add a few words for the agent, which is
  worth the typing: a silent refusal teaches them nothing.

If you turn something down, the agent's note is still written into the
property's private notes with a date. What they found out is never thrown away,
even when the timing is wrong.

**Nothing here is a back door.** Agents hold *Ask the office to change a property
that belongs to somebody else*; only you hold *Approve or turn down changes
proposed from the field*. An agent still cannot publish anything they were not
handed.

## 3.7 Societies & Projects
...
**Users** screen; see Part 3.14 for the two roles the plugin provides.
...
| Automated tests | 2,038 integration + 49 unit |
...
### 2.7.0 — what an agent can do when the website is wrong

A property in your office belongs to whoever added it. Until now, anybody else
who found something wrong on the ground — a sold sticker, a revised price — had
one option: tell a colleague, eventually.

Now they can say it there and then. They open the property, and instead of a
locked screen they get a short form: price, rent, whether it is still available,
the description, and one line about how they know. That arrives on **Change
Requests**, and on the office's Today list, with today's value and the proposed
value side by side. The office says **"Yes, change it"** or **"Not now"**.

Two things matter about that design:

- **Nothing changes without a decision.** A proposal is not a draft and not an
  edit. The website keeps showing what your office last confirmed until someone
  with the responsibility for it says yes.
- **A refusal keeps the information.** If the timing is wrong today, the agent's
  note is still written into the property's private notes, so what they found
  out is not thrown away.

Approving is not a special path. It saves through the same code the editor uses,
so the search index, the audit history and the "undo last edit" snapshot all
behave exactly as they would if you had typed the change yourself.

Agents get a permission called *Ask the office to change a property that belongs
to somebody else*; owners and managers get *Approve or turn down changes proposed
from the field*. You will find both on the Team screen. If you would rather not
hear about every request by email, the switch is in Office Settings under Emails,
...
Version 2.7.0 · This manual matches that version.
