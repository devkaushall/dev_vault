...
final class ListingsScreen {
	...
	private static function editor( int $listing_id ): void {
		...
		if ( ! Requests::may_edit_directly( $listing_id ) ) {
			// One screen, one honest path: what they cannot do is not offered,
			// and what they can do is right there rather than behind a wall.
			self::propose( $listing_id );
			return;
		}
	...
	/**
	 * The one thing an agent can do with a property that is not theirs.
	 *
	 * Refusing them outright is the wrong answer. Standing in a flat that has
	 * just changed hands, they know something the website has wrong, and a
	 * locked screen means the website stays wrong until a colleague gets back.
	 * So the record is shown as it is, and the handful of facts a person on the
	 * spot can genuinely confirm are offered as a request.
	 *
	 * @param int $listing_id The listing being looked at.
	 * @return void
	 */
	private static function propose( int $listing_id ): void {
		$post = get_post( $listing_id );
		$data = Listings::to_array( $listing_id, true );

		Partials::card_open(
			/* translators: %s: property name. */
			sprintf( __( '%s belongs to somebody else', 'estat-os' ), (string) ( $post ? $post->post_title : __( 'That property', 'estat-os' ) ) ),
			__( 'You can read it, and you can ask for a correction. Nothing changes until the office says yes.', 'estat-os' )
		);

		echo '<dl class="estat-record-facts">';
		foreach (
			array(
				__( 'Price', 'estat-os' )            => (string) $data['price_display'],
				__( 'Still available?', 'estat-os' ) => (string) Vocabulary::label( 'availability', (string) $data['availability'] ),
				__( 'Size', 'estat-os' )             => (float) $data['area_sqft'] > 0 ? Format::area( (float) $data['area_sqft'], 'sqft' ) : '',
				__( 'Bedrooms', 'estat-os' )         => (int) $data['bedrooms'] > 0 ? (string) (int) $data['bedrooms'] : '',
				__( 'On the website', 'estat-os' )   => 'publish' === (string) $data['status'] ? __( 'Yes', 'estat-os' ) : __( 'Not yet', 'estat-os' ),
			) as $label => $value
		) {
			echo '<dt>' . esc_html( (string) $label ) . '</dt><dd>' . esc_html( '' !== trim( (string) $value ) ? (string) $value : '—' ) . '</dd>';
		}

		$desc = trim( (string) wp_strip_all_tags( (string) ( $post ? $post->post_content : '' ) ) );
		if ( '' !== $desc ) {
			echo '<dt>' . esc_html__( 'Description', 'estat-os' ) . '</dt><dd>' . esc_html( 400 < strlen( $desc ) ? substr( $desc, 0, 397 ) . '…' : $desc ) . '</dd>';
		}
		echo '</dl>';

		Partials::card_close();

		self::propose_form( $listing_id );
	}

	/**
	 * The short form that sends a correction to the office.
	 *
	 * Deliberately four fields, not the forty the editor has. Somebody on a
	 * phone in a lobby wants to fix a price and say "it is sold", not meet a
	 * form. Anything beyond that is office work.
	 *
	 * @param int $listing_id The listing being corrected.
	 * @return void
	 */
	private static function propose_form( int $listing_id ): void {
		$symbol = (string) Settings::get( 'currency_symbol', '₹' );

		Partials::card_open(
			__( 'Suggest a change', 'estat-os' ),
			__( 'Fill in only what is wrong. Leave a field alone and it stays as it is.', 'estat-os' )
		);
		?>
		<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=estat-listings&edit=' . $listing_id ) ); ?>" class="estat-request-form">
			<?php wp_nonce_field( 'estat_propose_request_' . $listing_id, 'estat_nonce' ); ?>
			<input type="hidden" name="estat_action" value="propose_listing" />
			<input type="hidden" name="listing_id" value="<?php echo esc_attr( (string) $listing_id ); ?>" />
			...
			foreach ( Requests::editable_fields() as $field => $meta ) {
				...
				self::propose_field( (string) $field, (string) $meta['type'], (string) ( $meta['label'] ?? '' ), $listing_id, $symbol );
			}
			?>

			<div class="estat-field-row">
				<label class="estat-field-label" for="estat-request-note">
					<?php esc_html_e( 'A note for the office', 'estat-os' ); ?>
					<span class="estat-required"><?php esc_html_e( 'required', 'estat-os' ); ?></span>
				</label>
				<textarea id="estat-request-note" name="message" rows="3" required maxlength="600" placeholder="<?php esc_attr_e( 'The owner told me this morning…', 'estat-os' ); ?>"></textarea>
				<p class="description"><?php esc_html_e( 'One line about how you know, so they can decide without calling you.', 'estat-os' ); ?></p>
			</div>

			<p class="estat-request-form-foot">
				<button type="submit" class="button button-primary"><?php esc_html_e( 'Send to the office', 'estat-os' ); ?></button>
				<a class="button-link" href="<?php echo esc_url( admin_url( 'admin.php?page=estat-listings' ) ); ?>"><?php esc_html_e( 'Back to listings', 'estat-os' ); ?></a>
			</p>
		</form>
		<?php
	...
	/**
	 * One proposed field, with what the record says today written under it.
	 *
	 ...
	 * @return void
	 */
	private static function propose_field( string $field, string $type, string $label, int $listing_id, string $symbol ): void {
		$id  = 'estat-request-' . sanitize_html_class( $field );
		$raw = Requests::current_value( $listing_id, $field );
		$now = Requests::describe( $field, $raw );

		$today  = 'Nothing entered' !== trim( $now ) ? $now : '';
		?>
		<div class="estat-field-row">
			<label class="estat-field-label" for="<?php echo esc_attr( $id ); ?>">
				<?php echo esc_html( $label ); ?>
				<span class="estat-optional"><?php esc_html_e( 'only if it is wrong', 'estat-os' ); ?></span>
			</label>
			<?php if ( 'availability' === $type ) : ?>
				<select id="<?php echo esc_attr( $id ); ?>" name="changes[<?php echo esc_attr( $field ); ?>]">
					<option value=""><?php esc_html_e( 'Leave as it is', 'estat-os' ); ?></option>
					<?php
					foreach ( Vocabulary::availability() as $value => $text ) {
						if ( $value === $raw ) {
							continue; // No point proposing what it already says.
						}
						echo '<option value="' . esc_attr( (string) $value ) . '">' . esc_html( (string) $text ) . '</option>';
					}
					?>
				</select>
			<?php elseif ( 'textarea' === $type ) : ?>
				<textarea id="<?php echo esc_attr( $id ); ?>" name="changes[<?php echo esc_attr( $field ); ?>]" rows="3" placeholder="<?php esc_attr_e( 'Type the whole new description, as plain text…', 'estat-os' ); ?>"></textarea>
			<?php else : ?>
				<input type="number" id="<?php echo esc_attr( $id ); ?>" name="changes[<?php echo esc_attr( $field ); ?>]" min="0" step="1000" inputmode="numeric" placeholder="<?php echo esc_attr( $symbol ); ?>" />
			<?php endif; ?>
			<p class="description">
				<?php
				printf(
					/* translators: %s: what the website shows today. */
					esc_html__( 'Today it says: %s', 'estat-os' ),
					esc_html( '' !== trim( $today ) ? $today : __( 'nothing yet', 'estat-os' ) )
				);
				?>
			</p>
		</div>
