...
final class Actions {
	...
	private const CAPS = array(
		...
		'propose_listing'    => 'estat_request_listings',
		'approve_request'  => 'estat_review_listings',
		'decline_request'  => 'estat_review_listings',
	...
	public static function handle(): void {
		...
		switch ( $action ) {
			...
			case 'propose_listing':
				self::propose_listing();
				break;
			case 'approve_request':
				self::settle_request( 'approve' );
				break;
			case 'decline_request':
				self::settle_request( 'decline' );
				break;
			default:
				self::tool( $action );
		}
	...
	/**
	 * An agent asks the office to change somebody else's property.
	 *
	 * The agent's own records go through save_listing() and are edited
	 * directly; this is only the route for a record they cannot touch. Which
	 * of the two applies is decided by Requests::may_edit_directly(), so the
	 * screen and this handler cannot disagree.
	 *
	 * @return void
	 */
	private static function propose_listing(): void {
		$record_id = self::int_post( 'listing_id' );
		self::verify( 'estat_propose_request_' . $record_id );

		// Gone between opening the page and pressing the button.
		if ( ! get_post( $record_id ) ) {
			self::redirect( admin_url( 'admin.php?page=estat-listings' ), 'error', __( 'That property could not be found. It may already have been removed.', 'estat-os' ) );
		}

		if ( Requests::may_edit_directly( $record_id ) ) {
			self::redirect( admin_url( 'admin.php?page=estat-listings&edit=' . $record_id ), 'error', __( 'You can change that property yourself, so there is nothing to ask for.', 'estat-os' ) );
		}

		// phpcs:disable WordPress.Security.NonceVerification.Missing
		$changes = isset( $_POST['changes'] ) ? (array) wp_unslash( $_POST['changes'] ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$message = isset( $_POST['message'] ) ? Sanitize::textarea( wp_unslash( $_POST['message'] ) ) : '';
		// phpcs:enable

		if ( '' === $message ) {
			self::redirect(
				admin_url( 'admin.php?page=estat-listings&edit=' . $record_id ),
				'error',
				__( 'Add one line about how you know, so the office can decide without calling you.', 'estat-os' )
			);
		}
		...
		if ( is_wp_error( $result ) ) {
			self::redirect(
				admin_url( 'admin.php?page=estat-listings&edit=' . $record_id ),
				'error',
				self::first_message( $result )
			);
		}

		self::redirect( admin_url( 'admin.php?page=estat-listings' ), 'requested' );
	}

	/**
	 * Settle one request: apply it, or turn it down with a word of explanation.
	 *
	 * Approval writes through Listings::save(), so validation, the search
	 * index, the audit trail and the undo snapshot all behave exactly as if the
	 * office had typed the change in themselves.
	 *
	 * @param string $verb 'approve' or 'decline', naming both the action and
	 *                     the nonce the screen printed. One word, so the two
	 *                     can never drift apart.
	 * @return void
	 */
	private static function settle_request( string $verb ): void {
		$request_id = self::int_post( 'request_id' );
		self::verify( 'estat_' . $verb . '_request_' . $request_id );

		// phpcs:ignore WordPress.Security.NonceVerification.Missing
		$reason = isset( $_POST['reason'] ) ? Sanitize::textarea( wp_unslash( $_POST['reason'] ) ) : '';
		// phpcs:enable

		$result = 'approve' === $verb
			? Requests::approve( $request_id )
			: Requests::decline( $request_id, $reason );

		if ( is_wp_error( $result ) ) {
			self::redirect( admin_url( 'admin.php?page=estat-requests' ), 'error', self::first_message( $result ) );
		}

		self::redirect( admin_url( 'admin.php?page=estat-requests' ), 'request_' . ( 'approve' === $verb ? 'approved' : 'declined' ) );
	}
