 ...
 *
 * Only the fields being changed are stored being changed, never a whole record. If two
...
final class Requests {
	...
	public static function create( array $input ) {
		...
		// A note alone is not a change. If every box was left blank or held a
		// value the sanitiser threw away (a price typed as words), saying
		// "nothing was filled in" is the truth, and far kinder than quietly
		// filing a request that will change nothing when it is approved.
		$value_changes = array_diff_key( $changes, array( 'note' => true ) );

		if ( ! $value_changes ) {
			return new WP_Error( 'estat_empty_request', __( 'Nothing was filled in that we could use, so there is nothing to ask for. A number must be a number.', 'estat-os' ) );
		}
		...
		do_action( 'estat_request_created', $request_id, $record_id );

		return $request_id;
	}

	/**
	 * Whether this person may change this record themselves.
	 *
	 * The office, and the agent a property belongs to, edit it directly.
	 * Anyone else has to ask. This is the only place that decision is made, so
	 * the screen offering a form and the save path accepting one can never
	 * disagree about who may do what.
	 *
	 * @param int $record_id Listing or project ID.
	 * @return bool
	 */
	public static function may_edit_directly( int $record_id ): bool {
		if ( $record_id <= 0 ) {
			return false;
		}

		return current_user_can( 'estat_manage_settings' ) || current_user_can( 'estat_edit_record', $record_id );
	}

	/**
	 * What the record says today, in the same shape a request carries.
	 *
	 * A reviewer cannot judge "change the price to 8500000" without the
	 * number it is replacing, and neither can the agent writing the request.
	 *
	 * @param int $record_id Listing or project ID.
	 * @return array<string,string>
	 */
	public static function current_values( int $record_id ): array {
		$current = array();

		foreach ( self::editable_fields() as $field => $meta ) {
			$current[ $field ] = self::current_value( $record_id, (string) $field );
		}
	...
	/**
	 * One current value, read from the record itself.
	 *
	 * @param int    $record_id Listing or project ID.
	 * @param string $field     Field name.
	 * @return string
	 */
	public static function current_value( int $record_id, string $field ): string {
		if ( $record_id <= 0 ) {
			return '';
		}
		...
		switch ( $field ) {
			case 'price':
			case 'rent':
				return (string) (float) get_post_meta( $record_id, '_estat_' . $field, true );

			case 'availability':
				return (string) get_post_meta( $record_id, '_estat_availability', true );
	...
	/**
	 * Readable text for one proposed or stored value.
	 *
	 * Numbers arrive as digits; nobody should have to compare "8500000" with
	 * "8250000" in their head, so money is shown the way the website shows it.
	 *
	 * @param string $field Field name.
	...
	public static function describe( string $field, string $value ): string {
		...
		if ( '' === $value ) {
			return __( 'Nothing entered', 'estat-os' );
		}

		$type = (string) ( $fields[ $field ]['type'] ?? '' );

		if ( 'money' === $type ) {
			return Format::money( (float) $value );
		}

		if ( 'availability' === $type ) {
			$choices = \EstatOS\Support\Vocabulary::availability();

			return (string) ( $choices[ $value ] ?? $value );
		}

		if ( 'textarea' === $type ) {
			return 240 < strlen( $value ) ? substr( $value, 0, 237 ) . '…' : $value;
		}
