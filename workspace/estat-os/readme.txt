...
just hidden from the menu. A property still belongs to whoever added it: an
agent cannot change somebody else's, but they can propose a correction from the
property itself, which the office approves or turns down.
...
= 2.7.0 =
* Agents can now propose a correction to a property that belongs to a
  colleague. The office approves or turns it down on the new Change Requests
  screen; nothing on the website changes until they do.
* A leftover entry in the action permission map is gone. Removing a property was
  (and remains) available on every listing row; the dead entry made an auditor
  believe deletion had no route at all.
