...
const cli = await runCLI({
  ...
  mount: [
    ...
    { hostPath: './scripts/phase9-privacy-runner.php', vfsPath: '/phase9-privacy-runner.php' },
  ],
  blueprint: { steps: [{ step: 'activatePlugin', pluginPath: '/wordpress/wp-content/plugins/realestate-platform/realestate-platform.php' }] },
});
try {
  const result = await cli.playground.run({ code: '<?php require \'/phase9-privacy-runner.php\';' });
  ...
  fs.writeFileSync(`verification-results/phase9-privacy-${php}.json`, JSON.stringify(parsed, null, 2) + '\n');
  ...
  fs.mkdirSync('verification-results', { recursive: true });
  fs.writeFileSync(`verification-results/phase9-privacy-${php}.log`, text);
  console.error(text);
  process.exitCode = 1;
} finally {
