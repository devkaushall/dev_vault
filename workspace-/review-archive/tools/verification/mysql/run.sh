...
HERE="$ROOT/tools/verification/mysql"; OUTDIR="$ROOT/evidence/phase-1/database/mysql"; OUT="$OUTDIR/lifecycle.json"
mkdir -p "$OUTDIR"
...
"${dc[@]}" wp core install --url=http://wordpress.test --title='Phase 1 MySQL' --admin_user=admin --admin_password='disposable-test-only' --admin_email=admin@example.test --skip-email
